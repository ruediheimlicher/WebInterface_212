...
//   [twiStatusDic setObject:[NSNumber numberWithInt:1]forKey:@"local"];
...
- (IBAction)reportLocalTaste:(id)sender;
{
  ...
  // [localStatusDic setObject:[NSNumber numberWithInt:1]forKey:@"local"];
...
- (IBAction)readEthTagplan:(id)sender
{
   ...
   NSLog(@"readEthTagplan raum: %ld objekt: %d wochentag: %d",(long)[RaumPop indexOfSelectedItem],[ObjektPop indexOfSelectedItem],[TagPop indexOfSelectedItem]);
...
- (void)readEthTagplanVonRaum:(int)raum vonObjekt: (int)objekt vonTag:(int)wochentag
{
   ...
   NSLog(@"readEthTagplanVonRaum: %d  objekt: %d wochentag: %d",wochentag,objekt, wochentag);
      //return;
...
- (void)FinishLoadAktion:(NSNotification*)note
{
	...
	if ([[note userInfo]objectForKey:@"okcode"]) 
	{
		...
		NSLog(@"FinishLoadAktion  objectForKey okcode ist vorhanden: %d",OK_Code_Status);
	...
	//			[PWFeld setStringValue:@""];
		...
		if (([[note userInfo]objectForKey:@"radrok"]) && ([[[note userInfo]objectForKey:@"radrok"]intValue]))
		{
			//Adresse_OK=[[[note userInfo]objectForKey:@"radrok"]intValue];
			// Anzeigen, dass die EEPROM-Adresse erfolgreich uebertragen wurde: 
			
			NSLog(@"FinishLoadAktion read EEPROM: radrok ist da Adresse_OK: %d TWI_Status: %d",Adresse_OK,TWI_Status);
			if (TWI_Status==0)
             {
      //          if (Adresse_OK)// Passwort OK
                {
                   //NSLog(@"radrok ist da");
...
#pragma mark writeok
   ...
   NSMutableArray* EEPROMDataArray = [[NSMutableArray alloc]initWithCapacity:0];;
   if ([[note userInfo]objectForKey:@"in_daten"]) 
   {
      NSLog(@"FinishLoadAktion EEPROM lesen neue Version");
      Data_OK=[[[note userInfo]objectForKey:@"in_daten"]intValue];
      // Anzeigen, dass die daten erfolgreich uebertragen wurde: 
      if ((TWI_Status==0)&&Data_OK)// Data OK
      {
         NSLog(@"FinishLoadAktion EEPROM lesen: objectForKey data ist vorhanden: data ist da");
         [[NSSound soundNamed:@"Tink"] play];
	...
	if ([[note userInfo]objectForKey:@"data"]) 
	{
		...
		if ((TWI_Status==0)&&Data_OK)// Data OK
		{
         NSLog(@"FinishLoadAktion EEPROM lesen: objectForKey data ist vorhanden: data ist da");
			...
			{
				//	[StatusFeld setStringValue:[[note userInfo]objectForKey:@"eepromdatastring"]];
            NSArray* EEPROMDataIntArray = [[[note userInfo]objectForKey:@"eepromdatastring"] componentsSeparatedByString:@"+"];
		//		EEPROMDataArray = [[[note userInfo]objectForKey:@"eepromdatastring"] componentsSeparatedByString:@"+"];
            for (int index = 0;index < 8;index++)
            {
               [EEPROMDataArray addObject:[NSString stringWithFormat:@"%02X",[[EEPROMDataIntArray objectAtIndex:index]intValue]]];
            }
